 ...
 * @author jahrralf
 */
public class DataParallelism extends ParallelDesignPattern {

    public DataParallelism(String title) {
        super(title);
    }

    /* Assume there are enough cores, then the longest stage is the thing to wait for (all should be equal) */
    /* @Override
     public int getOverallWeight(int mode) {
     if (mode == ActivityPatternDiagram.PARALLEL) {
     int max = Integer.MIN_VALUE;

     for (Activity a : stages) {
     max = Math.max(max, a.getOverallWeight(mode));
     }
    ...
    /* @Override
     ...
     for (Activity a : stages) {
     max = Math.max(max, a.getOverallWeight());
     }
     ...
     int max = 0;
     for (Activity a : stages) {
     max += a.getOverallWeight();
     }
     return max;
     }
     } */
    /**
     * DataParallelism can use any number of cores between 1 and MAX
     */
    @Override
    ...
    protected double getObjectiveValueDuration() throws ObjectiveException {
        ...
        /* if(getParameter().getValue() > 1) {
            ...
            result += packs[0].length * Platform.OVERHEAD_PER_ROUND_DP;
